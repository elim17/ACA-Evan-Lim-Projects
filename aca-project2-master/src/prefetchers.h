...
struct prefetcher {
    ...
    void *data;
    //TODO This was added as part of the custom prefetcher-------------
    // guess the address based on previous access pattern
    void *previousAddressArray;
    void *blockAddressArray;
    void *stateArray;
    uint32_t sizeOfReferancePredictionTable;
    uint32_t rptWriteHead;
    bool rptFull;
    uint32_t actualPreviousAddress;
    //
    //try to pick up on a stride to reduce compulsory misses
    int memAccessStride;
    uint32_t strideState;

    //Todo This must be coppied over---------------------------
};
